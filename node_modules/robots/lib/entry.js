...
Entry.prototype.allowance = function(url) {
  ...
  if (possibleRules.length > 0) {
    var longestRulePathLen = 0;
    var longestRule;

    possibleRules.forEach(function(rule) {
      var pathLen = rule.path.length;
      if (longestRulePathLen < pathLen) {
        longestRulePathLen = pathLen;
        longestRule = rule;
      }
    });

    return longestRule.allowance;
  }
